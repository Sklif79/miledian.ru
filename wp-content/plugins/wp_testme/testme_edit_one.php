        ...
        elseif ($testme_form_step == 2) {
            ...
            if (isset($test_data['question_text_old'])) {
                foreach ($test_data['question_text_old'] as $num => $question) {
                    ...
                    if ($question == "") {
                        ...
                        // сам вопрос
                        $wpdb->update($wpdb->testme_questions, array("question_text" => $question), array("ID" => $num));
                    }
                }
            }
            // Новые вопросы
            if (isset($test_data['question_text_new'])) {
                ...
                foreach ($test_data['question_text_new'] as $num => $value) {
                    ...
                    //вопрос
                    if ($value != '') {
                        $wpdb->insert($wpdb->testme_questions, array("question_text" => $value, "question_test_relation" => $testme_id), array('%s', '%d'));
