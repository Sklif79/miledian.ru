...
function unite_social(){
  ...
  $output .= unite_social_item(of_get_option('social_tumblr'), 'ВКонтакте', 'tumblr');
  ...
  $output .= unite_social_item(of_get_option('social_dribbble'), 'Одноклассники', 'dribbble');
...
function unite_social_item($url, $title = '', $icon = ''){
  if($url != ''):
    $output = '<!--noindex--><a class="social-profile" href="'.$url.'" target="_blank" rel="nofollow" title="'.$title.'"><!--/noindex-->';
...
function unite_footer_info() {         
   $output = '<br><a href="http://miledian.ru/obratnaya-svyaz" class="callback-btn" title="Обратная связь">Обратная связь</a>';
   echo $output;
}
